...
sub parse {
    ...
    while ( my $stanza = _get_term( $fh, \$pushback ) ) {
        ...
        push( @{ $node->{relationships} }, @{$isa_relationships} )
          if @{$isa_relationships};

        my $part_of_relationships =
          _term_to_part_of_feature_relationships($term);

        push( @{ $node->{relationships} }, @{$part_of_relationships} )
